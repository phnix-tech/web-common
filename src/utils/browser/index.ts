...
function isMobile (ua?: string) {
  ...
  return flag || isiOS(ua) || isAndroid(ua) || isWechat(ua) || isWeibo(ua);
}
...
function isGsmIos (ua?: string) {
  ...
  return isiOS(ua) && /com\.garmin\.gccm/i.test(ua);
}
...
function isGsmAndroid (ua?: string) {
  ...
  return isAndroid(ua) && /com\.garmin\.android\.apps\.gccm/i.test(ua);
}
...
function isGSM (ua?: string) {
  ua = ua || navigator.userAgent;
  return isGsmIos(ua) || isGsmAndroid(ua);
}
